...
def run():
    ...
    print("Prepare Time: ", (time.time() - start_pre) * 1000, " ms")
    ...
    while (video_helper.not_finished(cur_frame_counter)):
        ...
        if configs.NUM_JUMP_FRAMES == 0:
            ...
            if detection_loop_counter % configs.NUM_JUMP_FRAMES == 0:
                ...
                # here we don't need to detect the frame
        ...
        img = visualizer.drawing_tracking(frame_show, object_controller.instances, cur_frame_counter, show_temporal_information)
        video_helper.write_video(img)
        cur_frame_counter += 1
